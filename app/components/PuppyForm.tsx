...
const PuppyForm = (props: PuppyFormProps) => {
  ...
  return (
    <form onSubmit={(e) => handleSubmit(e)} className='flex flex-col items-stretch gap-2 text-base leading-relaxed text-gray-500 dark:text-gray-200'>
        ...
        <input ref={nameRef} type="text" id="puppyName" className="rounded-lg p-2 text-black" placeholder='name' defaultValue={props.puppyExists ? props.puppy!.name : '' } required/>
        <label htmlFor="puppyBreed" className='mt-2 pl-2'>Puppy Breed:</label>
        <input ref={breedRef} type="text" id="puppyBreed" className="rounded-lg p-2 text-black" placeholder='breed' defaultValue={props.puppyExists ? props.puppy!.breed : ''} required/>
        <label htmlFor="puppyBD" className='mt-2 pl-2'>Puppy Birthdate:</label>
        <input ref={bDRef} type="date" id="puppyBD" className="rounded-lg p-2 text-black" defaultValue={props.puppyExists ? props.puppy!.birthdate : ''} required/>
